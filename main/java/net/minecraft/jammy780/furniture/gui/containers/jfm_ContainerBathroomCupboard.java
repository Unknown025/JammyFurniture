...
public class jfm_ContainerBathroomCupboard extends Container
{
	...
	public jfm_ContainerBathroomCupboard(IInventory par1IInventory, IInventory par2IInventory)
	{
		...
		for (int j = 0; j < this.numRows; j++)
		{
			...
			{
				addSlotToContainer(new Slot(par2IInventory, i1 + j * 4, 54 + i1 * 18, 16 + j * 18));
			}
		}
		for (int k = 0; k < 3; k++)
		{
	...
	public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2)
	{
		...
		if (var4 != null && var4.getHasStack())
		{
			...
			{
				if (!mergeItemStack(var5, this.numRows * 4, this.inventorySlots.size(), true))
				{
					return null;
				}
			}
			else if (!mergeItemStack(var5, 0, this.numRows * 4, false)) 
			{
				return null;
			}
			if (var5.stackSize == 0)
	...
	public void onContainerClosed(EntityPlayer par1EntityPlayer)
	{
		...
		this.bathroomCupboardInv.closeInventory();
	}
}
